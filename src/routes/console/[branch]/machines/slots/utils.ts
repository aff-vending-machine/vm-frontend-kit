...
import { slotsData, draft } from './events';
...
export const regroupData = (origin: MachineSlot[], filter: FilterType, border: BorderType): SlotType[] => {
  if (origin.length === 0) {
    return [];
  }
...
export function isEdited(id: number) {
  const a = get(draft).find(s => s.id === id);
  const b = get(slotsData).data?.find(s => s.id === id);
...
export function isPassed5Seconds(date?: Date) {
  if (!date) return true;
...
export function showTime(date?: Date) {
  if (!date) return get(t)('common.field.never');
  return dayjs(date).locale(get(locale).split('-')[0]).fromNow();
}
